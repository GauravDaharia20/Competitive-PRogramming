...
#include<bits/stdc++.h>
using namespace std;
int help(int id ,vector<int> &price,int len,vector<vector<int>>&dp){
    ...
    int not_take = 0 + help(id-1,price,len,dp);
    int take = INT_MIN;
    int rl = id+1;
    if(rl <=len){
        take = price[id] + help(id,price,len-rl,dp);
    }
    return dp[id][len] =  max(take,not_take);


}
int cutRods(vector<int> &price, int n,int len)
{
	// Write your code here.
    vector<vector<int>>dp(n,vector<int>(len+1,-1));
    return help(n-1,price,len,dp);
...
int main()
{
        ...
        for(int id = 1;id<n;id++){
            for(int len = 0; len<=n;len++){
                int not_take = 0 + dp[id-1][len];
                int take = INT_MIN;
                int rl = id+1;
                if(rl <=len){
                    take = price[id] + dp[id][len-rl];
                }
                dp[id][len] = max(take,not_take);
            }
        }
        cout<<dp[n-1][n];
}